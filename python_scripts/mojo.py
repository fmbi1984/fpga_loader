...
def main():
    ...
    #Serial code
    try:
        ser = serial.Serial(args.mojo_tty, 115200, timeout=10)
        ser.flush()
    except:
...
def install_mojo(ser, bitstream, verbose, no_verify, ram):
    ...
    if ram:
        ...
        if verbose and  ret == 'R':
            ...
            print 'Mojo did not respond correctly! Make sure the port is correct (1)'
            sys.exit(1)
    ...
    if not ram and no_verify:
        ...
        if verbose and  ret == 'R':
            ...
            print 'Mojo did not respond correctly! Make sure the port is correct (2)'
            sys.exit(1)
    ...
    if not ram and not no_verify:
        ...
        if verbose and  ret == 'R':
            ...
            print 'Mojo did not respond correctly! Make sure the port is correct (3)'
            sys.exit(1)
    ...
    step = 100.0/length
    percentage = 0
    with open(bitstream, 'rb') as fileobj:
        for word in fileobj:
            for ch in word:
                ser.write(ch)
                #time.sleep(0.0005)
    ...
    if verbose and  ret == 'D':
        ...
        print '\nMojo failed to flash correctly'
        sys.exit(1)
